...
function TripsItem() {
	...
	return (
		<section className="item-section section mt-5">
			...
			<SectionHeading sectionTitle={getContentFromMap(contentMap, 'tripItem.title', 'Trip')}></SectionHeading>
				...
				<div className="item-section__info col-12 col-md-7">
					...
					<div className="item-section__user-role">
						<p className="user-role uppercase-text">
							{getContentFromMap(contentMap, 'form.common.label', 'Description')}
						</p>
					...
					<div className="item-section__boxes flex-lg-row gap-lg-5 flex-wrap flex-xxl-nowrap">
						<ItemDetailsCard
							title={getContentFromMap(contentMap, 'trip.pricePerPerson', 'price per person')}
							number={`$${trip.pricePerPerson}`}
						/>
						<ItemDetailsCard
							title={getContentFromMap(contentMap, 'common.duration', 'duration')}
							number={`${trip.duration}`}
						/>
						<ItemDetailsCard
							title={getContentFromMap(contentMap, 'trip.level', 'level of difficulty')}
							number={`${trip.levelOfDifficultyName}`}
						/>
						<ItemDetailsCard
							title={getContentFromMap(contentMap, 'common.season', 'season')}
							number={`${trip.seasonName}`}
						/>
					</div>
